...
public class ImageChooser extends JDialog implements ActionListener, TreeSelectionListener, MouseListener {
	...
	public ImageChooser(Resource root, Resource selected){
		...
		image_thumb = new JLabel(new ImageIcon(Image.DEFAULT_THUMB)); image_thumb.setBorder(BorderFactory.createLoweredBevelBorder());
		inner2.add(image_thumb);
		size = new JLabel("Dimensions: 0 x 0"); inner2.add(size);
		inner.add(inner2);
		c.add(inner);
		inner = new JPanel();
	...
	public void valueChanged(TreeSelectionEvent e) {
		...
		if(r instanceof Image){
			selected_image = (Image)r; image_thumb.setIcon(new ImageIcon(selected_image.getThumb()));
