...
public class MediaPlayer extends JFrame implements TreeSelectionListener, FrameListener, ActionListener {
	...
	public void setProject(Project p){
		...
		Resource r = p.getChild(2).getChild(1); boolean b = m.getRoot() != r;
		if(b){m.setRoot(r); tree.clearSelection(); player.setClip(null);}
		else{TreePath path = tree.getSelectionPath(); m.nodeStructureChanged(r);
		tree.setSelectionPath(path); if(tree.getRowForPath(path) == -1) player.setClip(null);}
	}
