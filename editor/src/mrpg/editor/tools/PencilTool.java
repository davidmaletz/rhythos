...
public class PencilTool implements Tool {
	...
	public PencilTool(WorldPanel w, TilesetViewer v, History h){world = w; viewer = v; history = h;}
	public void paint(Graphics g, int stX, int stY, int mouseX, int mouseY){
		...
		for(TilesetViewer.SelectionIterator i = viewer.iterator(); i.hasNext(); ){
			...
			tile.paint(g, 0, x*Tile.tile_size, y*Tile.tile_size, 0, 0, Tile.tile_size, Tile.tile_size, viewer);
		}
	...
	public void repaint(int stX, int stY, int mouseX, int mouseY){
		...
		world.repaint((int)Math.floor((mouseX*Tile.tile_size-1)*s-1), (int)Math.floor((mouseY*Tile.tile_size-1)*s-1),
				(int)Math.ceil((viewer.getSelectionWidth()*Tile.tile_size+2)*s+2),
				(int)Math.ceil((viewer.getSelectionHeight()*Tile.tile_size+2)*s+2));
	}
	...
	private void setTile(int stX, int stY, int mouseX, int mouseY){
		...
		world.repaint((int)Math.floor((mouseX-1)*(s*Tile.tile_size)), (int)Math.floor((mouseY-1)*(s*Tile.tile_size)),
				(int)Math.ceil((selWidth+3)*(s*Tile.tile_size)), (int)Math.ceil((selHeight+3)*(s*Tile.tile_size)));
	}
