...
public class AnimatedTile extends Tile {
	...
	public static class Quad extends AnimatedTile {
		...
		public static AnimatedTile createTile(Image image, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, Tile.Info info, int tile_size){
			int half_tile = tile_size>>1;
			...
			if(x) return new AnimatedTile.Horiz(image, x1, y1, x3, y3, info, tile_size);
			if(y) return new AnimatedTile.Vert(image, x1, y1, x2, y2, info, tile_size);
			return new AnimatedTile.Quad(image, x1, y1, x2, y2, x3, y3, x4, y4, info, tile_size);
		}
	}
}
