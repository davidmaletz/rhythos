...
class Map extends Sprite {
	...
	public function new(d:ByteArray){
		...
		max_level = 0; for(y in 0...h) for(x in 0...w){
		...
		} for(l in 0...max_level){var s:Sprite = new Sprite(); renderLevel(s.graphics, l); addChild(s);}
	}
	...
	public function renderLevel(g:Graphics, l:Int):Void {
		var last:Tilemap = null, data:Array<Float> = null;
		var end:Int = cells.length; for(i in 0...end){
			...
			data.push(t.getIndex()); data.push(c.getX()*Tile.tile_size); data.push(c.getY()*Tile.tile_size);
		} draw(g, last, data);
	}
